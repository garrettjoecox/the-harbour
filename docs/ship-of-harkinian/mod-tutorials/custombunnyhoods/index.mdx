...
:::note
When I've made my bunny hoods I've using Blender 4.1 with the HM64 2.30 fork of Fast64 I don't know if that info matters but it may be helpful to someone 
:::
...
:::note
**But for the Ears THIS IS VERY IMPORTANT **
...
We gotta add the extra bones to the skeleton for each ear to work from so select Link's skeleton in object mode, and duplicate it with ctrl D
Rename both the armature object (orange man icon) and the Armature (The green man that's a child) they can share a name, in my case it's `Bunnyskell`

<img src={image2} alt="Dupped renamed Skeleton" width="700" />

Either copy the hat bone or go into edit mode an add a bone(length don't matter I tend to keep mine small or close to links other bones), make sure the bone does not have "Connected" ticked then make the bone a child of Links head (bone010_gLinkChildHeadLimb)

Then position it at the base of one of your ears, The rotation of the bones matters as does their roll value
	Rotate it so the small part/tail of bone is facing in the same direction as link is facing in
	Their X axis should point up and the Z axis should point to their left, Y should be pointing in the direction of the bone

Now add another bone for the other ear, you can mirror the one you have, dupe it one and move it etc, add it however you want just make sure the rotations are the same
...
To check if the weights are good stay in edit mode, unselect all verts pick a bone in the list then press Select to see what verts have been weighted
...
<img src={image3} alt="Weight painted ears" width="1000" />

We should be good to export it now, select the skeleton

:::note
Sometimes the Microcode you use to export can create issues if you're having issues try a different microcode f3dex/lx or f3dex2, both of them seem good, f3dex3 is not supported in SHO
:::
...
For the Asset Include Path, **don't use any existing paths**, ie don't use something like objects/object_link_child, objects/object_link_boy_hoverboots or objects/object_link_boy_goron etc
...
Make sure it exports to it's own folder and not overwriting anything
...
The big thing with the bunny hood here is assigning these bones to these specific matrixes 
`0x0B000000` and `0x0B000040` these are very special matrixes that are only used by the bunny hood, this is what gives the ears "physics" 
they only work with the bunny hood equipped though so can't really add physics to any other body parts without the bunny hood equipped

Also most DLs use the parameter `G_MTX_LOAD` but bunnyhood's ears need to have the `G_MTX_MUL` parameter

Save your DL and double check **it does not have an extension**
...
edited bunny hoods with their own materials and resaved vanilla textures seem ok
