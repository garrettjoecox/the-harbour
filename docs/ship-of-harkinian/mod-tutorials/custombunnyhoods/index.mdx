...
Make a new text file in the `objects/object_link_child` folder and save it as `gLinkChildBunnyHoodDL` **MAKE SURE IT HAS NO EXTENSION, This name is very important** as it refers to the actual bunny hood in the game's files, if you have the wrong name nothing will show up in game
